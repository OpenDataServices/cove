...
@pytest.mark.parametrize(('source_filename', 'expected_text', 'not_expected_text', 'conversion_successful'), [
    ...
    ('badfile_all_validation_errors.json', ['"" is too short. Strings must be at least one character. This error typically indicates a missing value.',
                                            ...
                                            'Field amount is not a number. Check that the value doesn’t contain any characters other than 0-9 and dot (.). Number values should not be in quotes',
                                            'Amount as a number.',
...
def test_validation_error_messages(url_input_browser):
    ...
    for html in [
        ...
        'Field <code>amount</code> is not a number. Check that the value  doesn’t contain any characters other than 0-9 and dot (<code>.</code>).',
